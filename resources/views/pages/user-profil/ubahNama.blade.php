    ...
    <body>
        <div id="headerDaftar" class="headerDaftar">
            <div class="app-container bg-white">
                ...
                <div style="padding: 16px;">
                    <p class="css-j23ndwa">Masukkan Nama Anda dengan benar, agar mudah mengenalinya.</p>
                    <form method="post" action="{{ route('storeUbahNama') }}">
                        @csrf
                        <div style="margin-bottom: 30px;">
                            <div style="padding-top: 9px">
                                <div class="form-group">
                                    <label class="css-45ndkwa">Nama</label>
                                    <input type="text" id="inputNama" name="inputanNama" class="form-control" style="font-size: 13px;" placeholder="Masukkan Nama Anda" value="{{ Auth::user()->name }}">
                                    <span class="text-danger" style="font-size: 12px;font-weight: 500;">Pastikan Nama Baru tidak sama dengan Nama Sekarang.</span>
                                </div>
                            </div>
                        </div>
        ...
        <script>
            $(document).ready(function() {
                var initialValue             = $("#inputNama").val().trim();

                function checkInputAndToggleButton() {
                    var inputNama            = $("#inputNama").val().trim();
                    var inputanKosong        = !inputNama;
                    var isInputSameAsInitial = inputNama === initialValue;
