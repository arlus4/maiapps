    ...
    <body>
        <div id="headerDaftar" class="headerDaftar">
            <div class="app-container bg-white">
                ...
                <div style="padding: 16px;">
                    <p class="css-j23ndwa">Masukkan Alamat Anda dengan benar, agar mudah mengenalinya.</p>
                    <form method="post" action="{{ route('storeUbahAlamat') }}">
                        @csrf
                        <div style="margin-bottom: 30px;">
                            <div style="padding-top: 9px">
                                <div class="form-group">
                                    <label class="css-45ndkwa">Masukkan Alamat</label>
                                    <input type="text" id="inputAlamat" name="inputanAlamat" class="form-control" style="font-size: 13px;" placeholder="Masukkan Alamat Anda" value="{{ $global_user->alamat_detail }}">
                                </div>
                            </div>
                        </div>
        ...
        <script>
            $(document).ready(function() {
                var initialValue              = $("#inputAlamat").val().trim();

                function checkInputAndToggleButton() {
                    var inputAlamat           = $("#inputAlamat").val().trim();
                    var inputanKosong         = !inputAlamat;
                    var isInputSameAsInitial  = inputAlamat === initialValue;
